...
(P[p,]xa,ya,[p,]xb,yb,...;[p,]xn,yn  User's Program Pattern (G)  This command enables you to draw characters,
                                         symbols which are not in the character code charts (too long to explain,
					 look here: imgurlinkpastlater (if you see it i forgot to do so))
...
Kca,cb,...,cn                        Kana(Greek)            (G)[t] In "Character and Symbol" section, so perhaps
                                         draws/cuts the given Greek or special characters (IMGUR for specification)
Lp,                                  Line Type              (G)  Specifies the type of line (IMGUR past here) (like "-- - --" or
                                         ". . ." where p - parameter from 0 to 8 (IMGUR)

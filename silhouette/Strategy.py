...
class MatFree(object):
    ...
    def pt2idx(self, x, y):
        ...
        if k in self.points_dict:
            ...
            if self.verbose:
                print("{0:d} found as dup".format(idx), file=sys.stderr)
    ...
    def load(self, cut):
        ...
        for path in cut:
            ...
            for point in path:
                ...
                if len(new_path) == 0 or new_path[-1] != idx or not self.do_dedup:
    ...
    def link_points(self):
        ...
        for path in self.paths:
            ...
            for pt in path:
                if A is not None:
                    if 'seg' in self.points[A].attr:
                        self.points[A].seg.append(pt)
                    else:
                        self.points[A].seg = [pt]

                    if 'seg' in self.points[pt].attr:
                        self.points[pt].seg.append(A)
                    else:
                        self.points[pt].seg = [A]
                A = pt

    def subdivide_segments(self, maxlen):
        """
        Insert addtional points along the paths, so that
           no segment is longer than maxlen
        """
        if not self.do_subdivide:
            return
        ...
        for path_idx in range(len(self.paths)):
            ...
            for pt in path:
                if len(new_path):
                    A = new_path[-1]
                    dist_a_pt_sq = dist_sq(self.points[A], self.points[pt])
                    if dist_a_pt_sq > maxlen_sq:
                        dist = math.sqrt(dist_a_pt_sq)
                        nsub = int(dist / maxlen)
                        seg_len = dist / float(nsub + 1)
                        dx = (self.points[pt].x - self.points[A].x) / float(nsub + 1)
                        dy = (self.points[pt].y - self.points[A].y) / float(nsub + 1)
                        if self.verbose > 1:
                            print("pt{0:d} -- pt{1:d}: need nsub={2:d}, seg_len={3:g}".format(A, pt, nsub, seg_len), file=sys.stderr)
                            print("dxdy {0} {1} {2} {3} {4} {5}".format(dx, dy, "to", (self.points[pt].x, self.points[pt].y), "from", (self.points[A].x, self.points[A].y)), file=sys.stderr)
                        for subdiv in range(nsub):
                            sub_pt = self.pt2idx(self.points[A].x + dx + subdiv * dx,
                                                 self.points[A].y + dy + subdiv * dy)
                            new_path.append(sub_pt)
                            self.points[sub_pt].sub = True
                            if self.verbose > 1:
                                print("   sub {0} {1}".format(self.points[sub_pt].x, self.points[sub_pt].y), file=sys.stderr)
                new_path.append(pt)
            self.paths[path_idx] = new_path

    def mark_sharp_segs(self):
        ...
        for pt in self.points:
            ...
            if 'seg' in pt.attr:
                ...
                for l1 in range(ll):
                    A = self.points[pt.seg[l1]]
                    for l2 in range(l1 + 1, ll):
                        B = self.points[pt.seg[l2]]
                        if sharp_turn(A, pt, B, self.sharp_turn_fwd_ratio):
                            pt.sharp = True
                    if 'sharp' in pt.attr:
                        break
            else:
                print("warning: no segments in point {0:d}. Run link_points() before mark_sharp_segs()".format(pt.id), file=sys.stderr)

    def mark_sharp_paths(self):
        ...
        min_jump_sq = self.corner_detect_min_jump * self.corner_detect_min_jump
        dup_eps_sq = self.corner_detect_dup_epsilon * self.corner_detect_dup_epsilon

        idx = 1
        A = None
        B = None
        for path in self.paths:
            if B is not None and len(path) and dist_sq(B, self.points[path[0]]) > min_jump_sq:
            ...
            for iC in path:
                ...
                if A is not None and sharp_turn(A, B, C, self.sharp_turn_fwd_ratio):
                    B.sharp = True
    ...
    def append_or_extend_hard(self, seg):
        ...
        if 'output' not in self.__dict__:
            self.output = []
        if len(self.output) and self.verbose > 1:
            print("append_or_extend_hard... {0} {1}".format(self.output[-1][-1], seg), file=sys.stderr)
        if (len(self.output) > 0 and len(self.output[-1]) >= 2 and
                'sharp' not in self.output[-1][0] and
                'sharp' not in self.output[-1][-1]):
            # we could flip around the previous segment, if needed:
            if (self.output[-1][0].id == seg[0].id or
                    self.output[-1][0].id == seg[-1].id):
                # yes, flipping the previous segment, will help below. do it.
                self.output[-1] = list(reversed(self.output[-1]))
                if self.verbose:
                    print("late flip {0} {1}".format(len(self.output), len(self.output[-1])), file=sys.stderr)
            #
        #

        if len(self.output) > 0 and self.output[-1][-1].id == seg[0].id:
            self.output[-1].extend(seg[1:])
            if self.verbose > 1:
                print("... extend", file=sys.stderr)
        elif len(self.output) > 0 and self.output[-1][-1].id == seg[-1].id:
            # check if we can turn it around
            if 'sharp' not in self.output[-1][-1].attr and 'sharp' not in seg[-1].attr and 'sharp' not in seg[0].attr:
                self.output[-1].extend(list(reversed(seg))[1:])
                if self.verbose > 1:
    ...
    def append_or_extend_simple(self, seg):
        """adds a segment to the output list. The segment extends the previous segment,
           if the last point if the previous segment is identical with our first
           point.
           Otherwise, the segment is appended as a new path.
        ...
        if 'output' not in self.__dict__:
            self.output = []
        if len(self.output) and self.verbose > 2:
            print("append_or_extend_simple... {0} {1}".format(self.output[-1][-1], seg), file=sys.stderr)
    ...
    def shortcut_segment(self, A, B, C):
        ...
        if len(C.seg) == 2:
            ...
            print("shortcut_segment: point C obsoleted. A,B,C: {0} {1} {2} {3}".format(A, B, C, C.att()), file=sys.stderr)
    ...
    def subdivide_segment(self, A, B, C):
        ...
        """
        print("subdivide_segment A,B,C: {0} {1} {2} {3} {4} {5}".format(A, A.att(), B, B.att(), C, C.att()), file=sys.stderr)
    ...
    def output_add(self, A, B, cut=False):
        ...
        if self.verbose >= 1:
            if len(self.output):
                print("output_add {0} {1} {2}".format(self.output[-1][-1], A, B), file=sys.stderr)
            else:
                print("output_add {0} {1} {2}".format(None, A, B), file=sys.stderr)
        #
        print("\t....................................", file=sys.stderr)
        if len(self.output) > 30:
            sys.exit(2)

        if cut:
            self.output.append([A, B])
        else:
            self.output.append([A])  # quite useless....
            self.output.append([B])

    def _dump_all(self):
        """ dump all points in a readable way.
        """
        for iP in range(0, len(self.points)):
            pt = self.points[iP]
            if pt is None:
                continue
            a = pt.att() if pt else None
            print("{0} {1} {2} {3}".format(iP, ": ", pt, a))

    def process_pyramids_barrier(self, y_slice, max_y, left2right=True):
        ...
        while True:
            if A is None:
                ...
                A = Xf_bar.point()
                continue
            print("process_pyramids_barrier {0} {1} {2}".format(left2right, A, A.att()), file=sys.stderr)
            ...
            for Bi in A.seg:
                if Bi >= 0:  # segment already done
                    a_todo += 1
                    pt = self.points[Bi]
                    if A.y + self.min_segmentlen >= max_y and pt.y > A.y:
            ...
            if B is None:
                print("no more forward segments {0} {1}".format(A, a_todo))
                Xb_bar.find(A, start=0)
                if a_todo == 0:
                    self.points[A.id] = None  # drop A
                while True:
                    Ai = Xf_bar.next()
                    A = None
                    print("xx next Ai candidate {0}".format(Ai), file=sys.stderr)
                    if Ai is None:
                        break
                    A = Xf_bar.point()
                    print("xx next A candidate {0}".format(A), file=sys.stderr)
                    if A is None:
                        break
                    if not Xb_bar.ahead(A):
                        break
                    else:
                        print("process_pyramids_barrier jump: Ignored A, ahead of Xb_bar {0}".format(A), file=sys.stderr)
                print("process_pyramids_barrier jump to next A {0}".format(A), file=sys.stderr)
                continue  # just advance Xf_bar: jump
            print("segment to check a), b) {0} {1}".format(A, B))
            ...
            if False:  # fake to trigger check a)
                ...
                B.id = b_id
                B.seg = [1, 2, A.id, 3, 4]
                print("faked segment to check a), b) {0} {1} ".format(A, B))
            #

            subdividable_ab = bool(dist_sq(A, B) > self.min_subdivide_sq)
            ...
            if subdividable_ab and B.y > max_y:  # check a)
                ...
                print("B below barrier, C={0}".format(C))
                self.subdivide_segment(A, B, C)
                Xf_bar.insert(C)
                Xb_bar.insert(C)
                B, C = C, B
            #
            ...
            if (subdividable_ab and (
                ...
                print("check b), moving Xb_bar from A to B {0} {1} {2} {3} {4} {5}".format(A, B, Xb_a_idx, Xb_b_idx, Xb_bar.key(A), Xb_bar.key(B)))
                D = None
                for n in range(Xb_a_idx, Xb_b_idx + 1):  # sweep from A to B
                    pt = Xb_bar.point(n)
                    if pt.id != A.id and pt.id != B.id and ccw(A, B, pt):
                        D = pt  # found a D that is clearly left of AB.
                        break
                    else:
                        print("backsweep ignoring pt {0} {1}".format(pt, Xb_bar.key(pt)))
                #
                if D is not None:  # compute intersection of Xb_bar with [AB]
                    E = intersect_lines(D, XY_a((D.x + 1, D.y + 1)), A, B, limit2=True)
                    if E is None:
                        raise ValueError("finding a shadowed D failed:", A, B, D)
                    E = XY_a(E)
                    self.subdivide_segment(A, B, E)
                    Xf_bar.insert(E)
                    Xb_bar.insert(E)
                    if C is not None:
                        self.shortcut_segment(E, C, B)  # replace segments [EB], [BC] with [EC]
                        B, C = C, B
                    B, E = E, B
            ...
            if Xf_b_idx is None:  # Should never happen!
                print("Xf_bar.lookup(B)=None. B={0}".format(B))  # Okayish fallback, but find() may return
                Xf_b_idx = Xf_bar.find(B)  # a different point with the same key().
            print("line A,B: {0} {1} {2} {3} {4}".format(A, B, Xf_a_idx, Xf_b_idx, Xf_bar.point(Xf_b_idx)))
            F = None
            Xf_f_idx = None
            for n in range(Xf_a_idx, Xf_b_idx + 1):  # sweep from A to B (inclusive)
                ...
                if subdividable_ab and pt.id != A.id and pt.id != B.id and ccw(A, B, pt) == (not left2right):
                    ...
                    print("forward sweep ignoring pt {0} {1} {2} {3}".format(n, pt, pt.id, Xf_bar.key(pt)))
            #
            if F is not None:  # compute intersection of Xb_bar with [AB]
                ...
                if G is None:
                    raise ValueError("finding a shadowed G failed:", A, B, F, _F_back)
                G = XY_a(G)
                self.subdivide_segment(A, B, G)
                Xf_bar.insert(G)
                Xb_bar.insert(G)
                if E is not None:
                    pass
                    # FIXME: should s.shortcut_segment(...E) something here.
                self.output_add(A, G, cut=True)
                self.unlink_segment(A, G)
                Xf_bar.pos(Xf_f_idx)  # advance to F, further up on the same barrier as G
                A = Xf_bar.point()
            #
            else:
                self.output_add(A, B, cut=True)
                self.unlink_segment(A, B)
                Xf_bar.pos(Xf_b_idx)  # advance
                A = Xf_bar.point()
            print("advanced A to {0}".format(A), file=sys.stderr)

        #  barrier has moved all the way to the other end.
        print("barrier moved all the way {0} {1} {2} {3}".format(Xf_bar.points, max_y, A.att() if A else None, A.seg if A else None), file=sys.stderr)

    def process_simple_barrier(self, y_slice, max_y, last_x=0.0):
        ...
        """
        if self.verbose:
            print("process_simple_barrier limit={0:g}, points={1:d}, {2}".format(max_y, len(y_slice), last_x), file=sys.stderr)
            print("                max_y={0:g}".format(y_slice[-1].y), file=sys.stderr)
        ...
        for pt in y_slice:
            if pt is None:  # all segments to that point are done.
                continue
            if 'seg' not in pt.attr:  # shit happens
                continue
            for iC in pt.seg:
                ...
                if C is not None and C.y <= max_y:
                    if self.verbose > 1:
                        print("   segments.append {0} {1}".format(C, pt), file=sys.stderr)
                    ...
                    if max_x is None or max_x < pt.x:
                        max_x = pt.x
                    self.unlink_segment(C, pt)
                #
            #
        #

        left2right = self.decide_left2right(min_x, max_x, last_x)
        xsign = -1.0
        if left2right:
            xsign = 1.0
        ...
        for segment in segments:
            ...
            if 'sharp' in A.attr and 'seen' in A.attr:
                if 'sharp' in B.attr and 'seen' in B.attr:  # both sharp
                    iM = self.pt2idx((A.x + B.x) * .5, (A.y + B.y) * .5)
                    M = self.points[iM]
                    if xsign * A.x <= xsign * B.x:
                        self.append_or_extend_hard([M, A])
                        self.append_or_extend_hard([M, B])
                    else:
                        self.append_or_extend_hard([M, B])
                        self.append_or_extend_hard([M, A])
                else:  # only A sharp
                    self.append_or_extend_hard([B, A])
            else:
                if 'sharp' in B.attr and 'seen' in B.attr:  # only B sharp
                    self.append_or_extend_hard([A, B])
                else:  # none sharp
                    if xsign * A.x <= xsign * B.x:
                        self.append_or_extend_hard([A, B])
                    else:
        ...
        if 'output' not in self.__dict__:
            return 0
        return self.output[-1][-1].x

    def decide_left2right(self, min_x, max_x, last_x=0.0):
    ...
    def pyramids_barrier(self):
        ...
        """
        self.output = []
        if not self.do_slicing:
            for path in self.paths:
                self.output.append([])
                for idx in path:
                    self.output[-1].append(self.points[idx])

            return

        dir_toggle = True
        old_min_y = -1e10
        old_len_output = len(self.output)
        while True:
            # always recreate the barrier, so that newly added subdivision points are seen.
            Y_bar = Barrier(self.points, key=lambda a: a[1] if a else 0)
            while Y_bar.point() is None:  # skip forward dropped points
                ...
                if Y_bar.next() is None:  # next() returns an idx, except when hitting the end.
                    break
            min_y = Y_bar.point().y
            barrier_y = min_y + self.monotone_back_travel
            print("\t>>>>>>>>>>>>>>> new Y-slice between {0} {1}".format(min_y, barrier_y), file=sys.stderr)
            y_max_idx = Y_bar.find((0, barrier_y))
            self.process_pyramids_barrier(Y_bar.pslice(), barrier_y, left2right=dir_toggle)
            ...
            if old_len_output == len(self.output) and old_min_y == min_y:
                print("pyramids_barrier aborted: no progress, stuck at min_y={0}".format(min_y), file=sys.stderr)
                break
    ...
    def simple_barrier(self):
        ...
        sy = sorted(self.points, key=by_y_key)

        barrier_y = self.barrier_increment
        ...
        while True:
            ...
            if barrier_idx > old_idx:
                last_x = self.process_simple_barrier(sy[0:barrier_idx], barrier_y, last_x=last_x)
            if barrier_idx >= len(sy):
                break
            barrier_y += self.barrier_increment
        #

    def apply_overshoot(self, paths, start_travel, end_travel):
    ...
    def apply(self, cut):
        ...
        if self.pyramids_algorithm:
            ...
            self.mark_sharp_segs()
            self.simple_barrier()
        if not self.tool_pen and self.overshoot > 0.0:

...
def _bbox_extend(bb, x, y):
    ...
    if 'llx' not in bb or x < bb['llx']:
        bb['llx'] = x
    if 'urx' not in bb or x > bb['urx']:
        bb['urx'] = x
    if 'lly' not in bb or y > bb['lly']:
        bb['lly'] = y
    if 'ury' not in bb or y < bb['ury']:
        bb['ury'] = y


class SilhouetteCameo(object):
        ...
        if no_device is True:
            ...
            if sys_platform.startswith('win'):
                ...
                try:
                    ...
                    if e.errno == 13:
                        msg = 'If you are not running as root, this might be a udev issue.\n' \
                              'Try a file /etc/udev/rules.d/99-graphtec-silhouette.rules\n' \
                              'with the following example syntax:\n' \
                              'SUBSYSTEM=="usb", ATTR{{idVendor}}=="{0:04x}", ATTR{{idProduct}}=="{1:04x}", MODE="666"\n' \
                              '\n' \
                              'Then run \'sudo udevadm trigger\' to load this file.\n' \
                              'Alternatively, you can add yourself to group \'lp\' and logout/login.'.format(self.hardware['vendor_id'], self.hardware['product_id'])
...
def wait_for_ready(s, timeout=30, verbose=True):
    ...
    for i in range(1, int(timeout * .5)):
        if state == 'ready':
            break
        if verbose:
            print(" {0:d}/{1:d}: status={2}\r".format(i, int(timeout * .5), state), end='', file=sys.stderr)
        if not verbose:
            if state == 'unloaded':
...
def setup(s, media=132, speed=None, pressure=None, pen=None, trackenhancing=False, bladediameter=0.9, landscape=False, leftaligned=None):
    ...
    if media is not None:
        if media < 100 or media > 300:
            media = 300
        s.write("FW{0:d}\x03".format(media))
        if pen is None:
            if media == 113:
                pen = True
            else:
